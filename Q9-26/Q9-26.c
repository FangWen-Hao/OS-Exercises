/*!
    ...
    Programming Problem 9.26 from Operating System Concepts 9th Edition, Galvin.
...
extern const char * usage_instructions;
...
int rand_ref_str(int len, int *ref_str) {
    ...
    unsigned long seed = jenkins_mix(clock(), time(NULL), getpid());
    srand(seed);
...
*/
int ref_str_print(int *ref_str, int len) {
...
*/
int run_from_cmd(int *ref_str, int len, int npf) {
    int p0, p1, p2;
    int npf_l = 1, npf_h = 8;
    ...
    ref_str_print(ref_str, len);

    printf("# page frames | FIFO | LRU | OPT\n");

    for(int i = npf_l; i < npf_h; i++) {
        p0 = fifoAlgorithm(len, ref_str, i);
        p1 = lruAlgorithm(len, ref_str, i);
        p2 = optAlgorithm(len, ref_str, i);
        printf("%13d | %4d | %3d | %3d\n", i, p0, p1, p2);
    }
...
int main(int argc, char **argv) {
    ...
    if(get_positive_arg(argc, argv, &num_page_frames, arg_name) < 0) {
    ...
    if (argc == 3 && num_page_frames > 0) {
      // Usage: Q9-26 -npf 7
        ...
        run_from_cmd(ref_str, len, num_page_frames);
        free(ref_str);
        return 0;
    }
    ...
    if (get_boolean_arg(argc, argv, &use_rand, arg_name) < 0) {
        printf("Error getting %s arg.\n", arg_name);
        return 6;
    }

    if(argc == 4 && num_page_frames > 0 && use_rand) {
      // Usage: Q9-26 -npf 9 -rand
    ...
    if(get_positive_arg(argc, argv, &len, arg_name) < 0) {
        printf("Error getting %s arg.\n", arg_name);
        return 7;
    }

    if(argc == 6 && num_page_frames > 0 && use_rand && len > 0) {
      // Usage: Q9-26 -npf 10 -rand -rsl 31
        ref_str = calloc(len, sizeof(int));
    ...
    if (get_boolean_arg(argc, argv, &use_rs, arg_name) < 0) {
        printf("Error getting %s arg.\n", arg_name);
        return 8;
    }

    if (argc == 5 && num_page_frames > 0 && use_rs) {
        if(get_ref_str_len(argv[use_rs + 1], &len))
        // Assumes the reference string is provided after the -rs argument.
            return 2;
